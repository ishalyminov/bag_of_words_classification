...
def get_statistics(in_texts_root):
    ...
    for (bag, answer) in zip(documents_set.get_bags(), documents_set.get_answers_vector()):
        partitioner.process_distribution(bag, 10)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        exit('Usage: classify.py <texts root folder>')
    for chunk_stats in get_categorized_statistics(os.path.abspath(sys.argv[1])):
        print '================\n%s' % chunk_stats[0]
